...
class MyTest extends tink.unit.Case.BasicCase {
	public function new() {
		super();
		info = {
			description: 'MyTest',
	...
	override function execute():Stream<Assertion> {
		assert.isTrue(true);
		assert.isTrue(false);
		return assert.complete();
	}
}
...
class RunTests {
	static function main() {
		...
		Runner.run([
			{info: {name: 'MySuite'}, cases: [new MyTest()]}
		]).handle(function(o) {
			travix.Logger.exit(0);
		});
		
		haxe.Timer.delay(function() trace('delayed'), 1000);
		// var code = 0;
		// var futures = [];
